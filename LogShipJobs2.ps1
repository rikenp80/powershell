...
{ 
    ...
    $TrimPos = $PrimaryReplicaName.indexof("\")
    $PrimaryServerName = $PrimaryReplicaName.substring(0, $TrimPos)

    $SourcePath = "\\" + $PrimaryServerName + "\epro\" + $db + "\*.trn"
    $TargetPath = $Backup_TargetPath + "\*.trn"
